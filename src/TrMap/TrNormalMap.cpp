...
void TrNormalMap::update(TrMap* map) {
  ...
  for (int i = 0; i < m_rows; i++) {
    for (int j = 0; j < m_cols; j++) {
      // l->r
      dvec3 lr(2.0, 0.0, 255.0 * (map->m_height->at(i, j + 1) -
                                         map->m_height->at(i, j - 1)));
      // u->d
      dvec3 ud(0.0, 2.0, 255.0 * (map->m_height->at(i + 1, j) -
                                         map->m_height->at(i - 1, j)));

      // // l->r
      // dvec3lr(2.0, 0.0, 255.0 * (map->m_height->at(i,j+1) +
      // map->m_water->at(i,j+1) -
      //                            map->m_height->at(i,j-1) -
      //                            map->m_water->at(i,j-1)));
      // // u->d
      // dvec3ud(0.0, 2.0, 255.0 * (map->m_height->at(i+1,j) +
